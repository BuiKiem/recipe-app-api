...
class PrivateRecipeApiTestCase(TestCase):
    ...
    def test_recipes_limited_user(self):
        """Test retrieve recipes of the authenticated user."""
        user2 = get_user_model().objects.create_user(email='other@example.com', password='password')
    ...
    def test_full_update_recipe(self):
        """Testing updating a recipe with 'put' method."""
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
        payload = {
    ...
    def test_filter_recipes_by_tags(self):
        """Return recipes with specific tag."""
...
class RecipeImageUploadTestCase(TestCase):
    ...
    def test_upload_invalid_image_to_recipe(self):
        """Upload an invalid image is failed."""
        url = image_upload_url(self.recipe.id)
